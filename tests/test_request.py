...
class TestNotification(TestCase):
    def test(self):
        self.assertEqual({"jsonrpc": "2.0", "method": "get"}, Notification("get"))

    def test_str(self):
        self.assertEqual(
            '{"jsonrpc": "2.0", "method": "get"}', str(Notification("get"))
        )

    def test_method_name_directly(self):
        self.assertEqual({"jsonrpc": "2.0", "method": "cat"}, Notification.cat())

    def test_positional(self):
        self.assertEqual(
            {"jsonrpc": "2.0", "method": "sqrt", "params": [1]}, Notification("sqrt", 1)
        )
...
class TestRequest(TestCase):
    ...
    def test(self):
        self.assertEqual({"jsonrpc": "2.0", "method": "get", "id": 1}, Request("get"))

    def test_str(self):
        self.assertEqual(
            '{"jsonrpc": "2.0", "method": "get", "id": 1}', str(Request("get"))
        )

    def test_method_name_directly(self):
        self.assertEqual({"jsonrpc": "2.0", "method": "cat", "id": 1}, Request.cat())
    ...
    def test_auto_iterating_id(self):
        self.assertEqual({"jsonrpc": "2.0", "method": "go", "id": 1}, Request("go"))
        self.assertEqual({"jsonrpc": "2.0", "method": "go", "id": 2}, Request("go"))
    ...
    def test_custom_iterator(self):
        req = Request("go", id_generator = ids.random())
